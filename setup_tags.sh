...
printf "\n\n\n**** 以下各項目  直接按 Enter  表示  不做更改 ****\n\n\n"
pause "按任意鍵繼續 或 Ctrl + C 中斷...."

# 設定 PHP_VERSION
printf "\n\n==== 設定 PHP 版本 PHP_VERSION ====\n"
CURRENT_PHP_VERSION=$(cat .env |grep ^PHP_VERSION=)
printf "目前設定 $CURRENT_PHP_VERSION\n"
printf "可用之值：\n"
# 由 php*.ini 抓取 php 版本的部份
ls php-fpm/php*.ini | cut -c 12- | rev | cut -c 5- | rev
ans=""
...
printf "\n"
printf "完整 tag 列表可至 https://hub.docker.com/r/t301000/xoops.easy.dock.php-fpm/tags 查詢\n"
read -p "新設定值： " ans
if [[ "$ans" != "" ]]; then
    ans=PHP_TAG\=$ans
...
printf "\n"
printf "完整 tag 列表可至 https://hub.docker.com/r/t301000/xoops.easy.dock.caddy/tags 查詢\n"
read -p "新設定值： " ans
if [[ "$ans" != "" ]]; then
    ans=CADDY_TAG\=$ans
...
printf "\n"
printf "完整 tag 列表可至 https://hub.docker.com/r/t301000/xoops.easy.dock.mysql/tags 查詢\n"
read -p "新設定值： " ans
if [[ "$ans" != "" ]]; then
    ans=MYSQL_TAG\=$ans
